...
def train_conv_net(datasets,
    ...
    def convolve_user_statuses(statuses):
        ...
        status_count,_ = theano.scan(fn = sum_mat, sequences=statuses, outputs_info=T.constant(0,dtype='float32'))
    ...
    while (epoch < n_epochs):
        ...
        if shuffle_batch:
            ...
            for minibatch_index in renge(n_train_batches):
                cost_epoch = train_model(minibatch_index)
                set_zero(zero_vec)
                print(minibatch_index)
        train_losses = [test_model(i) for i in renge(n_train_batches)]
        train_perf = 1 - np.mean([loss[0] for loss in train_losses])
        val_losses = [val_model(i) for i in renge(n_val_batches)]
...
def as_floatX(variable):
    ...
    if isinstance(variable, np.ndarray):
        return np.cast['float32'](variable)
    return theano.tensor.cast(variable, 'float32')
...
def make_idx_data_cv(revs, word_idx_map, mairesse, charged_words, cv, per_attr=0, max_l=51, max_s=200, k=300, filter_h=5):
    ...
    #
    mTrain = np.array(mTrain, dtype='float32')
    mTest = np.array(mTest, dtype='float32')
